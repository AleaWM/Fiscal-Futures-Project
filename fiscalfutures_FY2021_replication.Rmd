...
exp_temp <-  exp_temp %>% 
  ...
  (object=="4431" | (object>"1159" & object<"1166")  ) ~ 1, # 4431 = easy to find pension items
   # objects 1159 to 1166 are other objects to check for in_ff=0 later
...
table(exp_temp$pension) 
table(exp_temp$in_ff)

```
Pension = 2 represents teacher retirement pension payments that were excluded from the fiscal futures analysis by default(in_ff=0) but should be included. Pension2_amt should be added to revenue side.
...
# remove pension items to avoid double counting
exp_temp <- exp_temp %>% 
