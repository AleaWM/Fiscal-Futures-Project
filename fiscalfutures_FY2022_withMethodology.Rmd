...
- *To do: DONE* add "& fund !=0183" to teacher retirement pensions code below  - FY22 AWM     
...
```{r eehc, eval = FALSE, include = FALSE}
...
```{r state-employee-healthcare, eval = FALSE, include = FALSE}
...
0572
```{r}
rev_temp %>% filter( fund == "0477")
rev_temp %>% filter( fund == "0477" & source == "0572") %>% group_by(fy) %>% summarise(sum = sum(receipts))
...
- State employee contributions (eehc from eehc2_amt) should be moved to Other revenues. State employer contributins should bedropped.   

- State pension contributions funded by bonds (pension_amt from pension2_fy22) should be added to Other revenues.  
