...
> There is a 600 million payment into org_name == "Retirement System PMT Admin"! I think we've included large pension expenditures in the past, should this be included??

> Plus 300 million pension stabilization payment from fund 0319, object == 1900 for lump sums and other purposes.
...
POBs <- exp_temp %>% filter(fund == "0825")
...
exp_temp <-  exp_temp %>% 
 ...
 # Was this: (object=="4431" & fund=="0473" &  (fy==2010 | fy==2011)) ~ 3, # teachers retirement system, 
  ...
  (object=="4431" | (object>"1159" & object<"1166") & fund != "0183" & fund != "0193"  ) ~ 1, # 4431 = easy to find pension items
   # objects 1159 to 1166 are all considered Retirement by Comptroller 
  # object == 1167 also appears to be Other Retirement but isn't used yet
 fund == "0319" & obj_seq_type == "19930005" ~ 1, #pension stabilization fund
                                        TRUE ~ 0)) 
...
exp_temp %>% 
  filter(agency == '901') %>%  group_by(fy) %>% summarize(expenditure = sum(expenditure)) %>%
  ggplot(aes(x=fy, y=expenditure)) +
  geom_line() + 
  labs (title = "Pension Expenditures", caption = "Includes objects 1160-1165, 1298, 1900, and 4431.
        Object = 4431 includes most pension expenditures.")
```
