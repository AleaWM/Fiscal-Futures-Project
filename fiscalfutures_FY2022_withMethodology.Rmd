...
> This code chunk above for dealing with group insurance means that healthcare costs need to be added to expenditures after other group names are assigned. 
...
```{r}
exp_temp %>% filter(fy == 2021) %>% group_by(group) %>% summarise(sum =sum(expenditure)/1000000) #%>% format(~, big.mark= ",",  scientific = F)

exp_temp %>% filter(fy == 2021 & group == "930") %>% group_by(agency) %>% summarise(sum =sum(expenditure)/1000000) #%>% format(~, big.mark= ",",  scientific = F)
```
...
```{r}
#rev_temp %>% filter(rev_type == "39") #%>% group_by(source_name_AWM,fy) %>% summarize(sum=sum(as.numeric(receipts),na.rm = TRUE)) %>% ggplot() + geom_line(aes(x=fy, y = sum, group = source_name_AWM ))
```
...
#write.xlsx(dataset_names, file = 'summary_file_AWM_v5.xlsx')
```
