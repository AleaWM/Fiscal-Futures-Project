...
```{r inspect-newdata, eval=FALSE, include=FALSE}
revenue_fy22 <- read_xlsx("Fis_Fut_Rev_2022.xlsx") %>% 
...
revenue_fy22 %>% group_by(rev_type, fy) %>% summarize(revenue = sum(receipts) )%>% pivot_wider(names_from = fy, values_from = revenue)


exp_fy22 <- read_xlsx("Fis_Fut_Exp_2022.xlsx") %>% 
  rename(fy = 'FISCAL YEAR',
         fund = 'FUND #',
         fund_name = 'FUND NAME',
         agency = 'AGENCY #',
         agency_name = 'AGENC .NAME',
         appr_org = 'DIVISION',
         org_name = 'DIVISION NAME',
         obj_seq_type = 'APPROPRIATION #',
         wh_approp_name = 'APPROPRIATION NAME',
         exp_net_xfer = 'NET OF TRANS AMOUNT',
         expenditure = 'EXPENDED THRU 7/26/22'
  ) 
...
```{r create-rev-csv-FY21, include=FALSE}
setwd("C:/Users/aleaw/OneDrive/Desktop/PhD Fall 2021 - Spring 2022/Merriman RA/Fiscal Futures FY2022/revenue")

allrevfiles21 = list.files(path = "C:/Users/aleaw/OneDrive/Desktop/PhD Fall 2021 - Spring 2022/Merriman RA/Fiscal Futures FY2022/revenue",  pattern = ".dta") %>%  lapply(read_dta) %>% bind_rows %>% filter(fy < 2022)
#Fy21: 62294 observations, 13 variables
#FY22: 65094 obs, 13 vars

#write_csv(allrevfiles21, "allrevfiles21.csv")
...
allexpfiles21 = list.files(path = "C:/Users/aleaw/OneDrive/Desktop/PhD Fall 2021 - Spring 2022/Merriman RA/Fiscal Futures FY2022/expenditures",  pattern = ".dta") %>%  lapply(read_dta) %>% bind_rows %>% filter(fy < 2022)

#fy21 213372 observations, 20 variables
# fy22 225587 obs, 21 vars.

#write_csv(allexpfiles21, "allexpfiles21.csv")
...
```{r}
agencies_past <- allexpfiles21  %>% filter(fy < 2022) %>% mutate(agency == as.character(agency)) %>% group_by(agency, agency_name) %>% unique() %>% summarize(Expenditure = sum(expenditure, na.rm = TRUE)) %>% drop_na() %>% arrange(-Expenditure)
...
anti_join(agencies22, agencies_past, by = c("agency", "agency_name")) %>% arrange(-Expenditure)




funds_past <- allexpfiles21  %>% filter(fy < 2022) %>% mutate(fund == as.character(fund)) %>% group_by(fund, fund_name) %>% summarize(count = n(), Expenditure = sum(expenditure, na.rm = TRUE))  %>% drop_na()
...
anti_join(funds22, funds_past, by = c("fund", "fund_name")) %>% arrange(-Expenditure)


orgs_past <- allexpfiles21  %>% filter(fy < 2022) %>% mutate(appr_org == as.character(appr_org)) %>% group_by(appr_org, org_name) %>% unique() %>% summarize(Expenditure = sum(expenditure, na.rm = TRUE)) %>% drop_na()
...
anti_join(orgs22, orgs_past, by = c("appr_org", "org_name")) %>% arrange(-Expenditure)


agencies_past <- allrevfiles21  %>% filter(fy < 2022) %>% mutate(agency == as.character(agency)) %>% group_by(agency, agency_name) %>% unique() %>% summarize(Receipts = sum(receipts, na.rm = TRUE)) %>% drop_na()
...
anti_join(agencies22, agencies_past, by = c("agency", "agency_name")) %>% arrange(-Receipts)


funds_past <- allrevfiles21  %>% filter(fy < 2022) %>% mutate(fund == as.character(fund)) %>% group_by(fund, fund_name) %>% summarize(count = n(), Receipts = sum(receipts, na.rm = TRUE))  %>% drop_na()
...
anti_join(funds22, funds_past, by = c("fund", "fund_name")) %>% arrange(-Receipts)



sources_past <- allrevfiles21  %>% filter(fy < 2022) %>% mutate(source == as.character(source)) %>% group_by(source, source_name) %>% summarize(count = n(), Receipts = sum(receipts, na.rm = TRUE))  %>% drop_na()
...
anti_join(sources22, sources_past, by = c("source", "source_name")) %>% arrange(-Receipts)
...
Pension2 used to be added to revenue side as All Other Revenue sources but as of Oct 2022, we can not determine why that was done and are no longer doing that.
...
filter(rev_temp) %>% 
  filter(rev_type == "78"& fy == 2021) %>%  write.csv("rev78-R.csv")
  #group_by(fund_name) %>% 
