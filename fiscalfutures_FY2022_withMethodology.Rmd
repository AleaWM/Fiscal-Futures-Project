...
> Note: `exp:1998_2022` and the `funds_ab_in_2022` dataframes have a fund_cat_name variable (AND THEY DONT MATCH 100%) which ends up creating a .x and .y version of the variable when they are joined together. Inspect this more later. It is not a huge concern because the fund number is what matters more.
...
~~POB-funded contributions to JRS, SERS, GARS, and TRS must be accounted for in a different way:~~

- Old and disputed: Pension = 2 represents retirement pension payments paid for with POB-funded contributions that were excluded from the fiscal futures analysis by default ( in_ff was 0 because bond financed funds are not sustainable cash flows) but should be included and added to the revenue side under "Other Revenues" in later steps. -- but why???  
...
```{r pensions1, include=FALSE, eval = FALSE}
#creates long version without any aggregation 
pension_2_long <- exp_temp %>% filter(pension == 2 ) %>% rename(year = fy) 

pension_2_long

pension2_fy22<- pension_2_long %>% group_by(year) %>% 
...
exp_temp %>% filter(group == "904") %>% group_by(fy) %>% summarise(healthcare_cost = sum(expenditure)) %>% arrange(-fy)

```

> This code chunk above for dealing with group insurance means that eehc no longer needs to be created or added into revenue at a later stage.

> Employee contributions/insurance premiums from the revenue side DO need to be subtracted from the total cost of employee healthcare for the net cost 

```{r eehc-old-method, eval = FALSE, include = FALSE}
...
```{r old-way, include = FALSE, eval=FALSE}
# old way, wrong:
# exp_temp <- exp_temp %>% 
...
- Include principle and interest for long-term debt that likely funds capital projects. Debt service is a proxy for capital depreciation and should be included as a cost. Tollway debt is longterm debt and is also included. 
...
- object == 8813 is for interest but obj_seq_type is used just to be more specific here.   
...
exp_temp <- anti_join(exp_temp, debt_drop) %>%
  mutate(
    agency = ifelse(fund != "0455" & (object == "8813" | object == "8800"| obj_seq_type == "88110008"), "903", as.character(agency)),
    group = ifelse(fund != "0455" & (object == "8813" | object == "8800"| obj_seq_type == "88110008"), "903", as.character(group)),
    in_ff = ifelse(group == "903", 1, in_ff))

# check work
exp_temp %>% filter(group == "903") %>% group_by(fy) %>% summarize(sum = sum(expenditure)) %>% arrange(-fy)
...
```{r Medicaid-check, include = FALSE}
...
- Subtract employee premiums from State Employee Healthcare expenditures  
...
pension_rev <- rev_temp %>% filter(rev_type == "51" & source == "0573" | source == "0572") %>% group_by(fy) %>%summarise(pension_rev_sum = sum(receipts)/1000000)
...
0120 = ins prem-opttional life/univ
...
0354 = optional health - retiremet hmo
...
Based on Alexis's comments - Look at items considered "Licenses and Fees":
```{r}
rev_temp %>% filter(rev_type == "39")
```
...
ff_exp<- ff_exp %>% select(-c(exp_904, exp_901, med_option_amt_recent, pension_rev_sum, exp_971:exp_976)) # drop unwanted columns
ff_exp
```
...
#write.xlsx(dataset_names, file = 'summary_file_AWM_v4.xlsx')
```
