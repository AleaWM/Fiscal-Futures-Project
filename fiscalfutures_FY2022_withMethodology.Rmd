...
pension_check <- exp_temp %>% 
  mutate(pension = case_when( 
    ...
    (object=="4431") ~ "Object 4431 - State Pension Contributions", # 4431 = state payments into pension fund
        (obj_seq_type > "11590000" & obj_seq_type < "11660000")  ~ "Retirement Objects; DROPPED",
  ...
  mutate(pension = case_when(
    ...
    (object=="1298" & (fy==2010 | fy==2011) & (fund=="0477" | fund=="0479" | fund=="0481")) ~ "Purchase of Investments", 
...
transfers_long %>% 
  ...
  summarize(expenditure = sum(expenditure, na.rm=TRUE) )%>% 
  ggplot() + geom_line(aes(x=fy, y = expenditure, color=agency_name)) + labs(title = "Transfers to Local Governments")
...
```{r tollway, include = FALSE}
tollway <- exp_temp %>% filter(fund == "0455")
tollway_debt <- exp_temp %>%filter(object == "8800")

#tollway debt principal and interest
exp_temp %>%filter(object == "8800") %>% group_by(fy) %>% summarize(sum=sum(expenditure)) %>% 
  ggplot() + geom_line(aes(x=fy, y=sum)) + labs(title = "Object 8800: Tollway Debt Principal and Interest")
...
exp_temp %>% filter(fund == "0455") %>% group_by(fy) %>% summarize(sum = sum(expenditure)) %>% ggplot() + geom_line(aes(x=fy, y=sum)) + labs(title = "Fund 0455 from Expenditure: Tollway Debt Service ", caption = "Data from IOC Expenditure Files. Fund 0455 is the IL State Tollway")


# is this for all revenues, not just bond proceeds?
rev_temp %>% filter(fund == "0455") %>% group_by(fy) %>% summarize(sum = sum(receipts))%>% 
  ggplot() + geom_line(aes(x=fy, y=sum)) + labs(title = "Fund 0455 from Revenue: Tollway Bond Proceeds", caption = "Data from IOC Revenue Files. Fund 0455 is the IL State Tollway Revenue")
