...
rev_type = 51 is for retirement/pension contributions from both employers and employees. These contributions from employers and employees are excluded from the Fiscal Futures model. However, it is still important to understand the money going in and out of then pension funds even if it is not included as a yearly expenditure or revenue. Retirement contribution revenue is graphed below.
```{r}
...
```{r}
rev_temp %>% filter(rev_type == "57" & fy == 2022) %>% group_by(source_name_AWM) %>% summarize(receipts =sum(receipts)) %>% arrange(-receipts)

rev_temp %>% filter(rev_type == "57" & fy == 2022 & source_name_AWM == "FEDERAL STIMULUS PACKAGE") %>% group_by(fund_name) %>% summarize(receipts =sum(receipts)) %>% arrange(-receipts)
...
The Fiscal Futures model focuses on sustainable revenue sources. To understand our fiscal gap and outlook, we need to exclude these one time revenues. GOMB has emphasized that they have allocated COVID dollars to one time expenditures (unemployment trust fund, budget stabilization fund, etc.). 
...
rev_long %>%
  ...
  labs(title = "2021 to 2022 Change in Revenue", x = "Millions of Dollars" , y = "",  caption = "")  +
   scale_fill_manual(values = c("#d62828", "#003049"), labels = c("FY 2021", "FY 2022"))+
    scale_color_manual(values = c("#d62828", "#003049")) +   theme_classic()+ 
    theme(
...
exp_long %>%
  ...
  mutate(Year = as.character(Year)) %>%
  ggplot(aes(x = Dollars, y = reorder(Category, Dollars))) +
  geom_line(aes(group = Category, alpha = 0.5) )+
    geom_text(aes(x = ifelse(Year == "2022", as.numeric(Dollars), NA),  label = ifelse(Year == "2022", Category_name, "")),  
            hjust = -0.2,
            size = 2.8) +
...
autoplot(tsexp) +
  ...
  labs(title = "Forecasted Revenue and Expenditures", caption = "dropped federal revenue from Covid response")
```

### Federal Revenue


__Note:__ These are graphed AFTER dropping the COVID response federal dollars. 
