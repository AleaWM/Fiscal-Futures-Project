...
# theme_Alea <- function(base_size = 14) {
...
rev_temp %>% filter(rev_type == "57" & fy > 2018 & source_name_AWM == "FEDERAL STIMULUS PACKAGE") %>% group_by(fund_name, fy) %>% summarize(receipts =sum(receipts)) %>% arrange(-receipts)
...
Corporate income tax Individual Income Tax Pass-Through (source =2582) was over 2 billion. The PTE tax allows a workaround to the federal $10,000 limitation for state and local tax (SALT) deductions and expires Jan 1. 2026 (to correspond with remaining years that the Tax Cuts and Jobs Act SALT limitation is in effect) [Tax Adviser](https://www.thetaxadviser.com/issues/2021/sep/illinois-passthrough-entity-tax-salt-cap-workaround.html). 
...
Implications: Illinois residents in multistate passthrough entities will need to pay estimated taxes on income that is not subject to the SALT cap tax.
TCJA of 2017 decreased 
...
ownsource_rev <- rev_long %>%
  filter(Category != "57" | Category != "58" | Category != "59") %>% group_by(Year) %>% summarize(Dollars = sum(Dollars))
