...
#write_csv(aggregated_totals_long, "aggregated_totals_fy2021.csv")
...
rm(cagr_1, cagr_2, cagr_3, cagr_5, cagr_10, cagr_23) # removes objects from computer memory
```
...
> Does not have a "Totals" row for table
...
Wide tables for summary file:
```{r wide-summary-tables}
revenue_wide <- pivot_longer(ff_rev, rev_02:rev_78new, names_to = c("type","Category"), values_to = "Dollars", names_sep = "_") %>% 
  rename(Year = fy) %>%
 # filter(Year <2022) %>%
  mutate(Category_name = case_when(
...
#    Category == "63" ~  "INVESTMENT INCOME", # other
 ...
 #   Category == "76" ~  "TRANSFER IN FROM OUT FUNDS",  #other
   ...
   # Category == "79" ~   "COOK COUNTY IGT", #dropped
 #   Category == "98" ~  "PRIOR YEAR REFUNDS", #dropped
                 T ~ "Other Revenue Sources **"

  ) ) %>% select(-type, -Category) %>% group_by(Year, Category_name) %>% summarise(Dollars= round(sum(Dollars),digits=2))
...
```{r wide-summary-tables2}
revenue_wide2 <- revenue_wide %>% pivot_wider(names_from = Category_name, 
              values_from = Dollars)

revenue_wide2

revenue_wide %>% pivot_wider(names_from = Year, 
              values_from = Dollars)
```


```{r wide-summary-table-expenditure}
expenditure_wide <- pivot_longer(ff_exp, exp_402:exp_970 , names_to = c("type", "Category"), values_to = "Dollars", names_sep = "_") %>% 
  rename(Year = fy ) %>% 
    filter(Year < 2022) %>%
          ...
          #   Category == "930" ~ "ELECTED OFFICERS" , 
           ...
           #  Category == "948" ~ "OTHER DEPARTMENTS" ,
            # Category == "949" ~ "OTHER BOARDS & COMMISSIONS" ,
             Category == "959" ~ "K-12 Education" ,
           #  Category == "960" ~ "UNIVERSITY EDUCATION",
             Category == "970" ~ "Local Government Revenue Sharing",
...
expenditure_wide2 <- expenditure_wide%>% pivot_wider(names_from = Category_name, 
              values_from = Dollars)
...
dataset_names <- list('Aggregate Revenues' = revenue_wide2, 'Aggregate Expenditures' = expenditure_wide2, 
                      'rev_long' = rev_long, 'exp_long' = exp_long, 
